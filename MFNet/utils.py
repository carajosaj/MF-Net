...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        print('\r{} Total time: {} ({:.4f} s / it)'.format(
...
def save_model(args, epoch, model_without_ddp, optimizer,
               save_ckpt_freq: int = 1, model_ema=None):
    ...
    if is_main_process() and isinstance(epoch, int):
        to_del = epoch - args.save_ckpt_num * save_ckpt_freq
        old_ckpt = output_dir / ('checkpoint-%s.pth' % to_del)
        if os.path.exists(old_ckpt):
            os.remove(old_ckpt)


def auto_load_model(args, model_without_ddp, optimizer):
    output_dir = Path(args.output_dir)
