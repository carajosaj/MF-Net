...
class KFNet(nn.Module):
    ...
    def make_layer(self,drop_path_rate: float = 0.):
        ...
        dp_rates = [x.item() for x in torch.linspace(0, drop_path_rate, len(cfgs))]

        for num, cfg in enumerate(cfgs):

            stages += nn.Sequential(
                KFNet_block(cfg[0], cfg[1], drop_rate=dp_rates[num], has_se=True, if_dw=True)
            )
            if cfg[2] == 1:
                downsample_layer = nn.MaxPool2d(kernel_size=2, stride=2)
